...
class Karma(object):
    ...
    _query_pattern = re.compile(r'^\s*memepoints\s+(\S+)\s*\?*\s*$')
    _dbname = u'karma'
    ...
    def process(self, nick, input):
        ...
        try:
            ...
            if nick.lower() != target.lower() and nick in ('veridiam', 'nkmucci', 'sydneyyay', 'conservativecat'):
                self.adjust(nick=target, adjustment=adjustment)
                karma = self.query(nick=target)
                kr.reply = u"%s has %s memepoints." % (target, karma)
            kr.matched = True
        ...
        try:
            ...
            karma = self.query(nick=target)
            kr.matched = True
            kr.reply = u"%s: %s has %s memepoints." % (nick, target, karma)
        except AttributeError:
            pass
        return kr
...
class Main(Module):
    ...
    help = u'\n'.join([
        u"<nick>[++/--] - adjust someone's memepoints",
        u"memepoints <nick> - see what someone's memepoints are",
        ])
